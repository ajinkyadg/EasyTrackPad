...
/// its own IOService via `MTDeviceCreateFromService`. An earlier version
/// of this app could only read one physical device at a time, because the
/// private enumeration API it used then (`MTDeviceCreateList`/
/// `MTDeviceIsBuiltIn`) never reliably delivered a Magic Mouse's frames;
/// targeting each device's exact IOService directly sidesteps that.
...
extension Trigger {
    ...
    /// implemented.
    public var supportsRepeatByDistance: Bool {
...
public struct CustomizationRule: Identifiable, Codable, Hashable {
    ...
    /// Per-rule overrides for the four sensitivity/repeat knobs that used
    /// to be single global sliders in Preferences (`SettingsStore
    /// .gestureSensitivity`/`repeatWhileHeldInterval`/`repeatWhileHeldDelay`/
    /// `repeatByDistanceSensitivity`). `nil` (the default for every one of
    /// these, and what every pre-existing saved rule decodes to) means
    /// "use the global default" — these only ever *narrow* behavior for a
    /// rule that explicitly opts in via `RuleFormView`, never change it
    /// for rules that don't. `sensitivityOverride` and
    /// `repeatByDistanceSensitivityOverride` only apply to `.trackpadGesture`
    /// triggers (see `GestureRecognizer`); `repeatIntervalOverride`/
    /// `repeatDelayOverride` only matter when `repeatsWhileHeld` is set.
    public var sensitivityOverride: Double? = nil
    public var repeatIntervalOverride: Double? = nil
    public var repeatDelayOverride: Double? = nil
    public var repeatByDistanceSensitivityOverride: Double? = nil
    ...
    public init(
        ...
        repeatsByDistance: Bool = false,
        restrictedToApps: [AppReference] = [],
        sensitivityOverride: Double? = nil,
        repeatIntervalOverride: Double? = nil,
        repeatDelayOverride: Double? = nil,
        repeatByDistanceSensitivityOverride: Double? = nil
    ) {
        self.id = id
        self.name = name
        self.device = device
        ...
        self.repeatIntervalOverride = repeatIntervalOverride
        self.repeatDelayOverride = repeatDelayOverride
        self.repeatByDistanceSensitivityOverride = repeatByDistanceSensitivityOverride
    }
